...
// Bootstrap 方法用于设置主机网络模式下的 CNI（容器网络接口）配置
// args: 传入的命令行参数
// pluginConfig: CNI 插件的配置信息
func (hostGW *HostGatewayCNI) Bootstrap(
	 ...
	 * 不过 cni-demo 这里暂时没有做 snat 的操作, 因为暂时没这个需求~
	 *
	 *
	 ...
	 *  2. 其他每台集群中的主机也添加
	 *  3. 把每台主机上的对外网卡都用 iptables 设置为可 ip forward: iptables -A FORWARD -i cni-demo0 -j ACCEPT
	 * 以上手动操作可成功
	 */
...
// Unmount 方法用于在主机网络模式下卸载 CNI（容器网络接口）配置
// args: 传入的命令行参数
// pluginConfig: CNI 插件的配置信息
...
// Check 方法用于检查主机网络模式下的 CNI（容器网络接口）配置是否正确
// args: 传入的命令行参数
// pluginConfig: CNI 插件的配置信息
...
// init 函数用于在包初始化时注册 HostGatewayCNI 结构体
func init() {
