...
func TestBird(t *testing.T) {
	...
	utils.DeleteFile("/opt/cni-demo/bird.cfg")
	err = GenConfigFile(is)
	test.Nil(err)
	test.True(utils.FileIsExisted("/opt/cni-demo/bird.cfg"))
	str, err := utils.ReadContentFromFile("/opt/cni-demo/bird.cfg")
	test.Nil(err)
	test.Equal(str, config)
	// return
	// utils.DeleteFile("/opt/cni-demo/bird.cfg")
	pid, err := StartBirdDaemon("/opt/cni-demo/bird.cfg")
	test.Nil(err)
