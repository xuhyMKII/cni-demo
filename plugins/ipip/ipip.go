...
// initEveryClient 初始化 ipam 客户端, 并返回一个 IpamService 实例
...
// setFibTalbeIntoNs 为给定的 Veth 配置网络命名空间内的路由表
...
// setUpHostPair 设置并启动主机上的 Veth Pair
...
// setUpHostPairProxyArp 为主机上的 Veth Pair 启用 Proxy ARP
...
// setUpHostPairForwarding 为主机上的 Veth Pair 启用 IP 转发
...
// setLocalFibTable 为主机上的 Veth 设置本地路由表
...
// setPodNetwork 配置 Pod 网络命名空间，并创建 Veth Pair
...
// setHostNetwork 在主机网络命名空间中配置 Pod 网络
...
// setIpForIpip 为 IPIP 设备分配 IP 地址
...
// Bootstrap 方法用于配置和启动 IPIP CNI 插件
...
// Check 方法用于检查 IPIP CNI 插件的配置是否正确
...
// GetMode 方法返回 IPIP CNI 插件的模式
func (ipip *IpipCNI) GetMode() string {
	return MODE
}

// init 方法将 IPIP CNI 插件注册到 CNI 管理器
func init() {
	ipipCNI := &IpipCNI{}
